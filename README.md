...
# For Linux headless operation (required for MSCZ to MusicXML conversion)
...
### MSCZ conversion fails on Linux

If MSCZ to MusicXML conversion fails on Linux servers, ensure the required Qt platform environment variables are set for headless operation:
...
These variables must be set before starting Registra and tell Qt/MuseScore to run without a display server, which is required for headless operation.
